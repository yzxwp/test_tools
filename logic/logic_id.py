...
def get_message():
    ...
    dic = {"用户邮箱": [email], "身份证号码": [card], "用户姓名": [name], "电话": [phone], "统一社会征信码": [credit],
           "工商银行": [bank_gongshang],
           "农业银行": [bank_nongye]}
    return dic
...
def get_count():
    count = input("请输入一个大于0小于22的整数(获取多少条数据)：")
    # 判断输入值是不是整数
    try:
        count = int(count)
        if count>22 or count<=0:
            return get_count()
        # junage=isinstance(count , int)
        return count
    except:
        print('不符合要求,请重新输入一个大于0小于22的整数(获取多少条数据):')
        return get_count()


def writte(count):
    data = u'   用户邮箱 ---- 身份证号码 ---- 用户姓名 ---- 电话 ---- 统一社会征信码 --- 工商银行----- 农业银行'
    print(data)
...
def wrrite_csv(count):
    header = {"用户邮箱", "身份证号码", "用户姓名", "电话", "统一社会征信码", "工商银行", "农业银行"}
...
def wrrite_csv_by_pandas2(count):
    dic1 = {"用户邮箱": [], "身份证号码": [], "用户姓名": [], "电话": [], "统一社会征信码": [],
           "工商银行": [],
           "农业银行": []}
    for i in range(0, count):
        data = get_message()
